...
async def sell_btc():
    ...
    try:
        ...
        btc_amount = balance.get('free', {}).get('BTC', 0)
        print(f"   Found {btc_amount} BTC.")
        
        if btc_amount > 0:
            print(f"   Selling {btc_amount} BTC to USDT...")
            symbol = "BTC/USDT"
            
            try:
                order = await exchange.create_market_sell_order(symbol, btc_amount)
                print(f"   ✅ SOLD BTC! Order ID: {order['id']}")
