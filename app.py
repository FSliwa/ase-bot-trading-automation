...
# Startup/shutdown events for bot manager
@app.on_event("startup")
async def startup_event():
    """
    Start bot manager sync loop and position monitor on application startup.
    CRITICAL: This ensures bots are resumed after server restart.
    """
    import asyncio
    from bot.bot_manager import bot_manager
    
    logger.info("🚀 Starting ASE Trading Bot Backend...")
    
    # 1. First, resume all bots that should be running
    logger.info("📍 Step 1: Resuming bots from database state...")
    try:
        await bot_manager.resume_all_bots()
        logger.info(f"✅ Resumed {bot_manager.get_active_bot_count()} bots")
    except Exception as e:
        logger.error(f"Failed to resume bots: {e}")
    
    # 2. Start the sync loop to keep bots in sync with DB
    logger.info("📍 Step 2: Starting bot sync loop...")
    asyncio.create_task(bot_manager.start_sync_loop(interval_seconds=30))
    logger.info("✅ Bot sync loop started")
    
    logger.info("🎉 ASE Trading Bot Backend started successfully!")


@app.on_event("shutdown")
async def shutdown_event():
    """Stop all bots on application shutdown."""
    from bot.bot_manager import bot_manager
    
    logger.info("🛑 Shutting down ASE Trading Bot Backend...")
    logger.info("Stopping all trading bots...")
    await bot_manager.stop_all_bots()
    logger.info("✅ All bots stopped")
    logger.info("ASE Trading Bot Backend shutdown complete")
