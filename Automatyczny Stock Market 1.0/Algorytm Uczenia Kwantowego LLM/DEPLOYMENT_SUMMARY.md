...
**Server:** 185.70.196.55 (Ubuntu 24.04 LTS)  
**Login:** root / MIlik112!@4  
**Current Status:** Server online, awaiting deployment
...
```bash
# Copy this ENTIRE command and paste in VPS console:
...
Use the detailed instructions in: `manual_deployment_commands.sh`
...
1. **Access VPS Console**
   - Go to your hosting provider's console
   - Login as `root` with password `MIlik112!@4`

2. **Execute Deployment**
   - Copy the one-line command above
   - Paste it in the VPS console
   - Wait 5-10 minutes for completion

3. **Verify Success**
   - Run monitoring: `./monitor_deployment.sh`
   - Visit: http://185.70.196.55
   - Check health: http://185.70.196.55/health
...
- ✅ http://185.70.196.55 shows Trading Bot interface
- ✅ http://185.70.196.55/health returns JSON status
- ✅ http://185.70.196.55/docs shows API documentation
...
**If deployment fails:**
1. Check VPS console for error messages
2. Verify all dependencies installed correctly
3. Check service status: `systemctl status trading-bot-api`
4. Review logs: `journalctl -u trading-bot-api -f`

**Common Issues:**
- **Python 3.11 not found:** Run `apt install python3.11 python3.11-venv`
- **Service won't start:** Check file permissions and paths
...
1. **Configure API Keys**
   - Add Binance/Bybit/PrimeXBT credentials
   - Set OpenAI/Anthropic API keys for AI features

2. **Upload Full Trading Bot Code**
   - Transfer complete `bot/` directory
   - Update with your latest `balance_fetcher.py` and other modules

3. **Enable Advanced Features**
   - Configure real trading (disable testnet)
...
- ✅ `init_vps.sh` - Complete VPS initialization
- ✅ `deploy.sh` - Main deployment script  
- ✅ `manual_deployment_commands.sh` - Step-by-step commands
- ✅ `quick_deploy_command.sh` - One-line deployment
- ✅ `monitor_deployment.sh` - Deployment monitoring
- ✅ `ssh_key_setup.sh` - SSH configuration guide
