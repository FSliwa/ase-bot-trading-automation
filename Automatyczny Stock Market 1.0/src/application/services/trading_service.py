...
class TradingService:
    ...
    async def upsert_api_key(
        ...
        *,
        ...
        requires_passphrase = {"coinbase", "coinbase_pro", "bitget"}
        if exchange in requires_passphrase and not passphrase:
            pretty_name = exchange.replace("_", " ").title()
            raise CredentialsError(f"{pretty_name} API requires a passphrase")
