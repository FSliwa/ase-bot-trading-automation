...
from src.domain.entities.trade import OrderType, TradeRequest, TradeResponse
...
class BitgetAdapter(ExchangeAdapter):
    """Adapter that proxies trading operations to Bitget via CCXT."""
    ...
    def __init__(self, config: AdapterConfig):
        ...
        if not config.passphrase:
            raise CredentialsError("Bitget API requires a passphrase")
        ...
        if config.testnet:
            # CCXT handles sandbox routing for Bitget when sandbox mode is enabled
    ...
    async def fetch_balance(self) -> Mapping[str, Any]:
        try:
            ...
            raise CredentialsError("Invalid Bitget credentials") from exc
    ...
    async def place_order(self, request: TradeRequest) -> TradeResponse:
        ...
        params: dict[str, Any] = {}
        if request.order_type is OrderType.LIMIT and price is not None:
