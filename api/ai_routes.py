...
async def start_bot(bot_id: str, token_data: dict = Depends(verify_token)):
    try:
        ...
        with DatabaseManager() as db:
            ...
            # Get user's API key
            from bot.models_legacy import ExchangeAPIKey
            api_key = (
                db.session.query(ExchangeAPIKey)
                .filter(ExchangeAPIKey.user_id == user_uuid)
            ...
            if not api_key:
                raise HTTPException(
                    status_code=400, 
                    detail="No active API key found. Please add exchange API credentials first."
                )
            ...
            success = await bot_manager.start_bot_for_user(
                user_id=str(user_uuid),
                api_key=api_key.api_key,
                api_secret=api_key.api_secret,
                exchange_name=api_key.exchange or "binance",
                testnet=getattr(api_key, 'is_testnet', False),
                futures=True
            )
            
            if not success:
                # Bot might already be running
                if bot_manager.is_bot_running(str(user_uuid)):
                    logger.info(f"Bot already running for user {user_uuid}")
                else:
                    raise HTTPException(status_code=500, detail="Failed to start trading bot")
...
async def stop_bot(bot_id: str, token_data: dict = Depends(verify_token)):
    try:
        ...
        with DatabaseManager() as db:
            ...
            await bot_manager.stop_bot_for_user(str(user_uuid))
